...
{
	double num;
	numbins = 2; // we will pad with two zeros
	std::ifstream inFile;
	std::string buffer;
	int i;
    ...
    a.push_back(0); // pad with another zero
	...
	if (! inFile.fail()) {
		...
		prior.insert(prior.begin(),prior.at(0));
		prior.push_back(num);
	} else {
...
void epdu::fixCol(int j)
{
	...
	double rr, rowSum, sumnew, roughness, temp1, temp2, temp3, randoval, sumfixed = 0;
	RNG x;
	...
	for (int i1=0; i1<B; i1++) {
		...
		while (true) {
			...
			if (j==1) {
				temp1 = temprow[j-1]*k/rowSum;
			} else {
				temp1 = r[j-1][i1]*k/rowSum;
			}
			temp2 = temprow[j]*k/rowSum;
			temp3 = temprow[j+1]*k/rowSum;
